...
You probably will most likely only needing to be looking for extension dependencies, as the dependencies from `npm` that we use should already be bundled in the extension build.
...
### Git :badge[Built-in] :badge[Required]{variant=failure}
