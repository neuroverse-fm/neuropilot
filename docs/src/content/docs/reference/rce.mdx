...
Certain functions still will be returned asynchronously, regardless of Autopilot or Copilot. This is due to the required APIs returning `Promise`s and async `Thenable`s.
