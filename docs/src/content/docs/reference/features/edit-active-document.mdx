...
For all of those options except Insert, Neuro can choose to use either glob patterns or regex to search for the targeted text.
...
For some reason, this doesn't work if VS Code is not the currently focused window, so you will need to focus the window before Neuro's action goes through.
